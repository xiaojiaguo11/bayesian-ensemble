...
for (j in 1:length(trim_value)) {
  ...
  for (i in 1:length(baseForecasts_valid)) {
    ...
    mase_all[i] <- mean(mase_cal(ts_i[[1]]$x, actual_i,trimmed_mean[i,1:length(actual_i)]))
  }
...
for (i in 1:length(submission_M4)) {
  ...
  smape_all[i] <- mean(smape_cal(actual_i,trimmed_mean[i,1:length(actual_i)]))
  mase_all[i] <- mean(mase_cal(submission_M4[[i]]$x, actual_i,trimmed_mean[i,1:length(actual_i)]))
}
...
for (m in 1:length(z_star_value)) {
  ...
  for (i in 1:length(baseForecasts_valid)) {
    ...
    mase_all[i] <- mean(mase_cal(ts_i[[1]]$x, actual_i,metrically_trimmed_mean[i,1:length(actual_i)]))
  }
...
for (i in 1:length(submission_M4)) {
  actual_i <- as.numeric(M4[[i]]$xx)
  smape_all[i] <- mean(smape_cal(actual_i,metrically_trimmed_mean[i,1:length(actual_i)]))
  mase_all[i] <- mean(mase_cal(submission_M4[[i]]$x, actual_i,metrically_trimmed_mean[i,1:length(actual_i)]))
}
